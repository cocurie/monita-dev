/**
 * Monita Gateway v1.1 — 横河ブリッジHD案件専用（8CH対応）
 *
 * ★2026-08-05: case02_Gateway/firmware/gateway_v1.1（汎用・v3.03/4CH固定）から分岐。
 * 横河ver1.1フィールドユニット（project06_yokogawa/ver1.1、8CH構成、BLE_PKT_TYPE=0x11）
 * のペイロードはFW_VERSIONバイトが無くCH1〜8がpayload[2]から並ぶ独自レイアウトのため、
 * 汎用版のv3.03互換パース（CH1-4固定・payload[3]起点）とは非互換。本ファイルは
 * EXPECTED_PKT_TYPE・buildBatchQuery()のパケット詰め替えを横河8CH仕様に合わせて改修している。
 * 汎用版（NEXCO等、v3.03 PktType=0x03）と機能追従する場合はcase02_Gateway側を都度確認すること。
 *
 * ベース: Monita Gateway v1.1 — BLE/LoRa 受信 + LTE-M → GAS 送信（AC電源版）
 *
...
static uint8_t  const EXPECTED_PKT_TYPE  = 0x11;             // Monita 横河ver1.1 BLE の Pkt type
#endif
...
int buildBatchQuery(const FlexRecord* merged, int start, int n,
  ...
  // パラメータ（&m{i}=&p{i}=&r{i}=）をやめ、DeviceID(1B)+CH1〜8(各2B)＝17バイト/台
  // だけを1本の &d= 16進blobにまとめて詰める（1台あたり34 hex文字）。
  ...
  // ★横河ver1.1向け改修（8CH対応）: 汎用gateway_v1.1（4CH固定・v3.03のFW_VERSIONバイトを
  // 前提にした payload オフセット）から分岐。横河ver1.1のMSDはFW_VERSIONバイトが無く
  // payload[2]からCH1が始まる（PktType,DeviceID,CH1,CH2,...,CH8）。
  // Epoch(4B)+DeviceID(1B)+CH1-8(16B) = 21バイト/台 = 42 hex文字。
  String body = "";
  int count = 0;
  for (int i = start; i < n; i++) {
    const FlexRecord& rec = merged[i];
    if (rec.payloadLen < 18) continue;  // PktType+DeviceID+CH1-8に満たない不正レコードは送らない
    ...
    // "&d=" は先頭1回だけ付くので、2台目以降は純粋にchunk(42文字)分だけ伸びる
    size_t oneLen = (count == 0) ? (String(F("&d=")).length() + 42) : 42;
