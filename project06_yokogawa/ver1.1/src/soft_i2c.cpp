...
bool softI2CWriteReg16(uint8_t addr7, uint8_t reg, uint16_t val) {
  i2cStart();
  if (!i2cWriteByte((addr7 << 1) | 0))   { i2cStop(); return false; }
  if (!i2cWriteByte(reg))                 { i2cStop(); return false; }
  if (!i2cWriteByte((val >> 8) & 0xFF))   { i2cStop(); return false; }
  if (!i2cWriteByte(val & 0xFF))          { i2cStop(); return false; }
  i2cStop();
  return true;
}
