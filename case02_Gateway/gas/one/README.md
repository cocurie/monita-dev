...
- CH4: スキャン回数（回）。`0`は未計測を示すが、PIRイベント数を残すため行自体は保持。
  One v1.1（FW 0x05〜）はCH4の bit13〜14 に充電状態を同居させるため、`& 0x1FFF` でマスクして表示する（8191で飽和）。
- 電池電圧: `(3000 + encoded * 5) / 1000` V。`255`は空欄。
- 充電状態（R列、v11〜）: CH4の `(値 >> 13) & 3` を `日照なし/未接続` / `充電中` / `満充電` / `異常(CHRG/DONE同時)` で表示。
  v1.00基板（FW 0x04以前）は常に `日照なし/未接続` になる。
...
登録していない。CH1はひずみ（με）の実測値、未使用のCH2〜CH3は`-1`を欠測値として扱う。
CH4はOne v1.1（FW 0x05〜）では充電状態コード（0〜3）で、R列に表示名を出す。v1.00基板のFW 0x04以前は`-1`（R列は空欄）。
