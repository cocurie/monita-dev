---
title: Monita Gateway v1.21 ファームウェア（FW 100 / Deck子機の受信＋ダウンリンク対応）
domain: iot_device
tags: [gateway, nRF52840, SIM7080G, BLE, LoRa, E220-900T22S, LTE-M, DS3231, SD, GAS, DIP, downlink]
updated: 2026-09-22
---

# gateway_v1.21 — 基板 ver1.21 用（FW 100）
...
> ソフトの版は `GATEWAY_FW_VERSION`（… 97 → 98 → 99 → **100**）で表す。
...
## FW 100（2026-09-22）— 週次レビューの信頼性修正

2026-09-21 週次レビュー（`monita-reviews/2026-09-21.md`）の A14・A15・A18 を修正した。
**GAS（gateway_common）も同時に更新が必要**（A14。GAS 側だけ・FW 側だけでも害は無いが効果は片側分になる）。

- **A14 GASのエラーページを失敗として扱う。**以前は本文 512 バイト超（Google のエラーページ）を
  検出しても警告だけ出して成功扱いにし、再送キュー・報告キューから消していた。
  `lastGasSuccessMs` も更新されるので、アプリ層WDTも障害に気づけなかった。
  GAS 側は、再送で直る失敗（ロック待ち）でわざと例外を投げてエラーページを返すようにした
  （`waitLockOrThrowRetryable_`）。**限界：**モデムが chunked 応答の本文を読めないため、
  GAS が HTTP 200 で返す短い文字列（'ERROR: invalid d length' 等）は今も区別できない。
  テレメトリは GAS 側で退避・再処理しているので、書込み失敗では欠損しない。
- **A15 stop 中にアプリ層WDTで再起動しない。**stop 中は送信しないので WDT の起点が進まず、
  1.5周期後に再起動して stop が無音で解除されていた。stop 中に限り check_cmd の成功を起点にする。
  check_cmd も失敗し続ける（通信断）場合は従来どおり再起動し、その場合 stop は解除される。
- **A18 受信ストール対策のキックを間引き、受信データを捨てない。**子機が30秒黙るたび
  （平常時はほぼ毎回）電波を出して受信バッファを全部捨てていたため、その窓に届いた
  フレームを失っていた。バッファは捨てずに解析へ回し、電波を出すキックは有効フレームが
  来ない間 30秒→1分→2分→4分→8分と間隔を延ばす。RX再武装（電波なし）は30秒ごとのまま。
...
## FW 99 追補（2026-09-13 Codexレビュー対応・版数は据え置き）

FW 99 は未出荷のため版数を上げずに直した。

- **GAS への報告を積んだ順に送る。**以前は配列の空きスロットに積んで先頭から送っていたため、
  最終結果（downlink_result）が先に積んだ中間報告（downlink_sent）を追い越すことがあった。
  GAS 側で done → sent に戻り、完了した予約が再配信され続ける。
  通し番号 `order` の昇順で送り、最終結果を積むときは同じ予約の中間報告を消す。
  送信中に ACK が届いて同じスロットが再利用されても消さないよう、送信後は `order` で本人確認する。
- **予約行の数値欄を厳密に読む。**`toInt()` / `strtoul()` は `"12abc"` を 12 として通す。
  全文字が数字（childId は16進2桁）で桁数が上限以内の行だけを受理し、mode は `0` か `1` のみ。
- **Deck ACK の status が 0/1/2 以外なら破棄**して再送を続ける（壊れた応答で予約を止めない）。
- **Deck ACK の子機FW版を GAS へ渡す**（`&fw=`）。適用値13バイトの値域は GAS が子機と同じ規則で検査する。
