---
title: Monita Flex v3.20 — Sigfox/BLE/LoRa 計測スケッチ（PlatformIO）
domain: monita_dev
tags:
  - Monita Flex
  - v3.20
...
**v3.10_lora をベースに、LoRaダウンリンク受信（Gatewayからの設定変更）を追加する版。**
`COMM_MODE_LORA` のときだけ有効で、BLE / Sigfox ビルドの挙動は v3.10 と変わらない。
ダウンリンクの設計・実機検証の経緯は `case01_Flex/test_sketches/25_lora_downlink_child`（子機側の実験台）を参照。

対応するGatewayファーム: `case02_Gateway/firmware/gateway_v1.20`
...
**Monita Flex v3.10 基板**（v3.03基板に、LoRa用にTCA9534(U6)のP2を新規配線したもの）。

v3.03基板にもそのまま書き込み可能（`COMM_MODE_SIGFOX`/`COMM_MODE_BLE`ビルドはピン割当が同一）。ただし`COMM_MODE_LORA`ビルドはP2が配線されたv3.10基板でのみ動作する。

★2026-08-11: ver3.10.sch / ver3.20.schのネットリストを直読みして確認した結果、**M0とM1はP2の1本で共通駆動**（基板上で短絡）されている。当初はP2=M0・P3=M1の2本独立を想定していたが、実基板の配線に合わせてファーム（`loraSetMode()`等）はP2共通駆動で実装済み。本READMEの以前の版に残っていた「P2/P3の2本独立」という記述は誤りだったため、この節で訂正した。
...
| TCA9534 config レジスタ | `0xFC`（P0/P1のみ出力） | LoRaビルド時はP0〜P2を出力（P0/P1=MUX A/B、P2=E220 M0/M1共通駆動）。P3〜P7は入力 |
| UART(D8/D9) | Sigfox専用 | Sigfox または **LoRa**（ビルド時排他で共用） |
| その他（I²C・HX711・D0） | v3.03 と同一 | 変更なし |

## LoRa（E220-900T22S(JP)）まわりの実装メモ

- **M0/M1制御**: TCA9534(U6) P2の1本でM0・M1を共通駆動（基板上で短絡。直結GPIOではなくI2C拡張GPIO経由。詳細はコード内コメント参照）
