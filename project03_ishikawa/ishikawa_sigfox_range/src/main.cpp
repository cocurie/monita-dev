/**
 * ★project03_ishikawa 専用版★
 *   ベース: case01_Flex/v3.20/src/main.cpp（コピー元と完全に同じ日付のスナップショット）
 *   変更点: Sigfoxペイロードのスロット5・6を [CH1レンジ, 気温] に差し替え
 *          （標準は [気温, 電池電圧]。電池電圧はこの版では送信しない）
 *          → 詳細はこのファイル内 sendSigfox() 直前のコメント、および README.md 参照
 *
...
// ★project03_ishikawa版: Sigfoxペイロードでも CH1レンジ（chRange[0]）を使うため、
//   ベース版（BLE/LoRaのみ）と異なり全モードで計算する。
int chRange[4];
...
// ============================================================
// Sigfoxペイロード構成（project03_ishikawa版、12バイト）
//
// ベース（v3.20標準）は [CH1-4] + [温度] + [電池電圧] だが、本案件向けに
// スロット5・6を差し替える（電池電圧は送信しなくなる点に注意）。
//
// | バイト | 内容                         |
// |--------|------------------------------|
// | 0-1    | CH1 ひずみ（με）              |
...
// | 4-5    | CH3 ひずみ（με）              |
// | 6-7    | CH4 ひずみ（με）              |
// | 8-9    | CH5: CH1レンジ（max-min、με） |
// | 10-11  | CH6: 気温（℃×10）            |
// ============================================================

// ch[0..3]・CH1レンジ・温度を連結した 16 進ペイロードで AT$SF= を送信
static void sendSigfox() {
  ...
  payload += hx4(chRange[0]);  // CH5: CH1レンジ（max-min）
  payload += hx4(tempV);       // CH6: 気温
