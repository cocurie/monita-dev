...
static const float STRAIN_SCALE = 1120.0f;  // 1.0f = 生値そのまま出力（未キャリブレーション）
...
// D0 ボタン押下 ISR（スリープ中に押されると vTaskDelay を早期終了させる）
...
// chRange[0..3]: HX711チャネルのMEASURE_COUNT回（平均値群）の最大-最小（ブレ幅）[με]
// YCE向けSigfoxペイロードでCH1・CH3のレンジを送信するため全モードで有効化
int chRange[4];
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (CH_ASSIGN[i] == 1) {
      ...
      if (tcaSelect((uint8_t)i) != 0) {
        ...
        ch[i] = measureVoltage(CH_ASSIGN[i] == 7);
      }
      tcaDisable();
    } else {
...
static void sendSigfox() {
  ...
  payload += hx4(chRange[0]);  // CH1レンジ
  payload += hx4(chRange[2]);  // CH3レンジ
...
void setup() {
  ...
  // ※ GPIOTE 有効化によりスリープ電流は約14µA増加する（実測 17→32µA 程度）。
