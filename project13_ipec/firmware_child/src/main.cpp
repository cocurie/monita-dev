 ...
 * | ERR_MCP9600_I2C        | MCP9600/9601（熱電対）の検出／初期化／読み取り失敗 | 同上     | 同上                         |
 *
 * 共通ポリシー:
...
// 上位3bit=Gateway群(0〜7) / 下位5bit=群内の機器番号(1〜31)。詳細は下の static_assert 参照。
#ifndef DEVICE_ID
#define DEVICE_ID 0x0F                    // LoRaモードの既定値（群0・機器15。iPEC案件 Flex②＝熱電対+ひずみ+変位計）
                                          // ※0x0E は既存のiPEC実機テスト用に使用中のため重複を避けた
#endif
static const uint8_t  FW_VERSION = 2;     // 子機ファームのバージョン。コミットのたびに+1すること
                                          // ★本ファームは case01_Flex/v3.20 からの派生。バージョンは
                                          //   本ファーム独自の連番であり、v3.20 の番号とは無関係（1から開始）
#endif
...
//   5 = MCP9600 / MCP9601（熱電対アンプ、I2C）※ TCA9546A 経由。熱電対タイプは
//       MCP9600_TC_TYPE で選択（本案件はT型）。case01_Flex/v3.10_lora から移植
//
// ★アイペック案件 Flex② の構成:
//   CH1 = ひずみゲージ / CH2 = ひずみゲージ / CH3 = ひずみ式変位計 / CH4 = T型熱電対
//   ひずみゲージと変位計はどちらも HX711 経由のため CH_ASSIGN は同じ 1。
//   換算係数だけ STRAIN_SCALE[] で CH 別に持てるようにしてある。
...
// ★アイペック案件で配列化。ひずみゲージ（CH1/CH2）と ひずみ式変位計（CH3）は
//   どちらも HX711 経由で計測経路は同一だが、感度が異なるため CH 別の係数を
//   持てるようにしておく。初期値は全CH同じ（v3.20 の実測値 1110.0f）。
//   キャリブレーション後に変位計CHだけ別係数にしたくなっても、ソース構造を
//   触らずに済む。1.0f = 生値そのまま出力（未キャリブレーション）。
static const float STRAIN_SCALE[4] = {1110.0f, 1110.0f, 1110.0f, 1110.0f};

// 0除算・負値ガード。未設定(0)や不正値のときは生値をそのまま返す扱いにする。
static inline float strainScaleOf(int i) {
  if (i < 0 || i > 3) return 1.0f;
  float sc = STRAIN_SCALE[i];
  return (sc > 0.0f) ? sc : 1.0f;
}
...
enum : uint32_t {
  ...
  ERR_MCP9600_I2C = 1u << 8,  // MCP9600/9601（熱電対）検出／初期化／読み取り失敗
};
...
// ============================================================
// MCP9600 / MCP9601（熱電対、I2C）— 温度[℃×10]を返す
// 熱電対タイプは MCP9600_TC_TYPE で選択（本案件はT型）。
//
// project06_yokogawa/ver1.1/src/main.cpp から移植。レジスタ定義・
// 初期化シーケンス（SENSORCONFIG=タイプ+フィルタ、DEVICECONFIG=0x80）は
// Yokogawa側と同一（ただしタイプはK固定ではなくマクロ化した。下記参照）。YokogawaはESP32-C3のI2Cクロックストレッチ制約
...
// ── 熱電対タイプ選択（SENSORCONFIG bit6-4）────────────────────
// MCP9600 の Thermocouple Type Select:
//   000=K / 001=J / 010=T / 011=N / 100=S / 101=E / 110=B / 111=R
//   → シフト済みの値: K=0x00 / J=0x10 / T=0x20 / N=0x30 / S=0x40 / E=0x50 / B=0x60 / R=0x70
//
// 本案件（アイペック）は **T型** 熱電対（CHINO VT6 相当）を使う。
// 移植元の case01_Flex/v3.10_lora はK型固定だった。
// ビルド時に上書きできる: PLATFORMIO_BUILD_FLAGS="-D MCP9600_TC_TYPE=0x00" → K型
#ifndef MCP9600_TC_TYPE
#define MCP9600_TC_TYPE 0x20   // T型
#endif
#define MCP9600_FILTER_MID 0x03  // フィルタ mid（bit2-0=011）。移植元と同じ
#define MCP9600_SENSORCFG_VALUE (MCP9600_TC_TYPE | MCP9600_FILTER_MID)

// 起動ログで型を取り違えていないか確認するための表示名
#if   MCP9600_TC_TYPE == 0x00
#define MCP9600_TC_TYPE_NAME "K"
#elif MCP9600_TC_TYPE == 0x10
#define MCP9600_TC_TYPE_NAME "J"
#elif MCP9600_TC_TYPE == 0x20
#define MCP9600_TC_TYPE_NAME "T"
#elif MCP9600_TC_TYPE == 0x30
#define MCP9600_TC_TYPE_NAME "N"
#elif MCP9600_TC_TYPE == 0x40
#define MCP9600_TC_TYPE_NAME "S"
#elif MCP9600_TC_TYPE == 0x50
#define MCP9600_TC_TYPE_NAME "E"
#elif MCP9600_TC_TYPE == 0x60
#define MCP9600_TC_TYPE_NAME "B"
#elif MCP9600_TC_TYPE == 0x70
#define MCP9600_TC_TYPE_NAME "R"
#else
#error "MCP9600_TC_TYPE は 0x00/0x10/0x20/0x30/0x40/0x50/0x60/0x70 のいずれかを指定してください"
#endif
...
// DEVICEID確認（0x40=MCP9600 / 0x41=MCP9601）→ 熱電対タイプ設定 → DEVICECONFIG書込
static bool mcp9600Init(uint8_t addr) {
  ...
  // SENSORCONFIG: 熱電対タイプ(bit6-4) + フィルタmid(bit2-0=011)
  // ★本案件（アイペック）はT型熱電対のため 0x20|0x03 = 0x23 を書く。
  //   移植元 v3.10_lora はK型固定で 0x03 を直書きしていた。
  Wire.beginTransmission(addr);
  Wire.write(MCP9600_REG_SENSORCFG);
  Wire.write((uint8_t)MCP9600_SENSORCFG_VALUE);
  if (Wire.endTransmission() != 0) return false;
  Serial.print("[MCP9600] SENSORCONFIG=0x");
  Serial.print((uint8_t)MCP9600_SENSORCFG_VALUE, HEX);
  Serial.print(" (");
  Serial.print(MCP9600_TC_TYPE_NAME);
  Serial.println("型)");
...
// 通常速度(100kHz)へ戻す。
// ★2026-09-07 実機調査で判明: この 10kHz 指定は nRF52 では効いていない。
//   Adafruit nRF52 コアの TwoWire::setClock() は baudrate<=100000 をすべて
//   TWIM_FREQUENCY_K100 に丸める（nRF52840 の TWIM は 100k/250k/400k の3段階しか
//   持たないハードウェア制約）。つまり実際は 100kHz のまま動作している。
//   → にもかかわらず MCP9601 の読み取りは成功している（2026-09-07 実機確認、28.88℃）。
//   移植元 v3.10 のコメントにある「100kHz では NACK で失敗する」は、
//   少なくとも本案件のモジュール・配線では再現していない。
//   低速化が本当に必要になった場合は、Wire ではなく
//   project06_yokogawa/ver1.1/src/soft_i2c.h のビットバング I2C を移植すること
//   （Yokogawa は本番稼働実績あり）。setClock では下げられない。
    ...
    if (CH_ASSIGN[i] == 1) {
      if ((s_errors & ERR_TCA9534_I2C) != 0U) {
...
#if defined(COMM_MODE_BLE) || defined(COMM_MODE_LORA)
        {
          ...
          chRange[i] = (int)(range / strainScaleOf(i));
        }
#else
        hxRead(&ch[i]);
#endif
        // 生値 → ひずみ値（με）変換
        ch[i] = (int)((float)ch[i] / strainScaleOf(i));
      }
      ...
      // TCA9546A のチャネル i（0 origin）を選択して MCP9600/9601（熱電対）を読む
      if (tcaSelect((uint8_t)i) != 0) {
        s_errors |= ERR_TCA_I2C;
        statusErrorRed();
        ch[i] = 0;
        ...
        // ★移植元 v3.10_lora のコメント: 切替直後すぐだと検出できない実機事例が
        //   あるため、待ち時間を 50ms→500ms に伸ばしてある。
        delay(500);
        ...
        // ★重要: scanI2CBus() はアドレス1〜126を総当たりする過程で TCA9546A 自身の
        //   アドレス(0x70)にも「データなしの書き込み」を送ってしまい、チャンネル選択が
        //   解除される。実機で確認済み（2026-09-07、スキャン有効時のみ mcp9600ReadTemp()
        //   が nan を返した）。スキャン後は必ずチャンネルを選び直すこと。
        tcaSelect((uint8_t)i);
#endif
        ch[i] = measureThermocouple();
      }
      tcaDisable();
    } else {
