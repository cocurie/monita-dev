 ...
 * | ERR_VL53L4CD_I2C       | VL53L4CD の初期化／測距失敗                | 赤・点灯 | 送信は行わない→スリープ    |
 * | ERR_MCP9600_I2C        | MCP9600（熱電対）の検出／初期化／読み取り失敗 | 同上     | 同上                         |
 * | ERR_ADS1115_I2C        | ADS1115（電圧入力）の読み取り失敗          | 同上     | 同上                         |
 *
 * 共通ポリシー:
...
// ============================================================
// ★2026-09-23 雛形: 案件ごとの値は project_config.h に集約（DeviceID・送信間隔・CH割当・換算係数など）
...
static const uint8_t  DEVICE_ID  = CFG_DEVICE_ID;  // ★2026-09-23 雛形: project_config.h で設定
// ★2026-09-23 雛形: v3.10_lora（FW 10）から分岐した雛形系列。ダウンリンク受信・可変長ペイロード(0x06)を追加したため +1。
static const uint8_t  FW_VERSION = 13;    // 子機ファームのバージョン。コミットのたびに+1すること（★2026-09-24 雛形FW13:設定を置換renameで保存）
#endif
...
// 消費電力への影響はない）。
#define SLEEP_MINUTES        CFG_SLEEP_MINUTES  // ★2026-09-23 雛形: 初期値。LoRa ではダウンリンクで変更可（flash 保存）
...
const uint8_t CH_ASSIGN[4] = CFG_CH_ASSIGN;  // ★2026-09-23 雛形: project_config.h で設定
...
static const float STRAIN_SCALE = CFG_STRAIN_SCALE;  // ★2026-09-23 雛形: project_config.h で設定（1.0f = 生値そのまま）

// HX711 1回の平均を求めるための生サンプル数（有野川子基板と同じ2段方式）
#define SAMPLES_PER_AVG CFG_SAMPLES_PER_AVG  // ★2026-09-23 雛形: 初期値。LoRa ではダウンリンクで変更可
// 平均値を何回取得してメジアンを求めるか（最大・最小はペイロードに含めない）
#define MEASURE_COUNT   CFG_MEASURE_COUNT  // ★2026-09-23 雛形: 初期値。LoRa ではダウンリンクで変更可
// ★2026-09-23 雛形: メジアン用配列の上限（実行時の回数は可変なので上限で確保する。v3.20 と同じ値）
...
enum : uint32_t {
  ...
  ERR_MCP9600_I2C = 1u << 8,  // MCP9600（熱電対）初期化／読み取り失敗
  ERR_ADS1115_I2C = 1u << 9,  // ADS1115（電圧入力）読み取り失敗
  ERR_DS18B20 = 1u << 10,     // ★2026-09-23 雛形: DS18B20 未接続（従来は 0 を返すだけでエラーにならなかった）
};

// ★2026-09-23 雛形: チャネル単位の計測で立つエラー。これが計測中に立ったチャネルは「欠測」として送る。
//   ERR_TCA9534_I2C（基板全体）は含めない（別途 HX711 チャネルを欠測扱いにする）。
static const uint32_t CH_ERR_MASK = ERR_HX711_TIMEOUT | ERR_MPU_I2C | ERR_TCA_I2C |
                                    ERR_VL53L4CD_I2C | ERR_MCP9600_I2C | ERR_ADS1115_I2C | ERR_DS18B20;
...
// ★2026-09-23 雛形: 以下の実行時設定・computeWdtTimeoutMs() は firmware_child（v3.20系）から移植。
//   送信間隔をダウンリンクで変えられるようにしたため、WDT も定数ではなく実行時の値から算出する。
// ============================================================
...
// ★2026-09-23 雛形: v3.20 系（/flex_settings.bin）とはファイル名を分ける。同じ基板に両方を書き込んでも
//   互いの保存内容を読まないようにするため。
static const char SETTINGS_FILE[] = "/tpl310_settings.bin";

// ★2026-09-23 雛形FW12: 保存の成否を返す（Codexレビュー指摘#2）。
//   旧実装は、flash の初期化・書き込みに失敗しても何も返さず、呼び出し側は成功として確認応答を返し、
//   送信間隔の変更なら再起動していた。再起動後は旧値（または初期値）に戻るのに、GAS には「完了」と
//   新しい値が残る。さらに先に既存ファイルを消していたので、途中で失敗すると旧設定も失っていた。
//   → 一時ファイルに書いて読み戻しで一致を確認してから、正式なファイルと差し替える。
static const char SETTINGS_TMP_FILE[] = "/tpl310_settings.tmp";

static bool saveSettings() {
  if (!InternalFS.begin()) { Serial.println("[SETTINGS] ✗ flashの初期化に失敗"); return false; }
  InternalFS.remove(SETTINGS_TMP_FILE);
  {
    File f(InternalFS);
    if (!f.open(SETTINGS_TMP_FILE, FILE_O_WRITE)) { Serial.println("[SETTINGS] ✗ 一時ファイルを開けない"); return false; }
    size_t n = f.write((const uint8_t*)&s_settings, sizeof(s_settings));
    f.close();
    if (n != sizeof(s_settings)) { Serial.println("[SETTINGS] ✗ 書き込み長が不一致"); return false; }
  }
  {
    File f(InternalFS);
    ChildSettings chk;
    if (!f.open(SETTINGS_TMP_FILE, FILE_O_READ)) { Serial.println("[SETTINGS] ✗ 読み戻しで開けない"); return false; }
    int n = f.read((uint8_t*)&chk, sizeof(chk));
    f.close();
    if (n != (int)sizeof(chk) || memcmp(&chk, &s_settings, sizeof(chk)) != 0) {
      Serial.println("[SETTINGS] ✗ 読み戻した内容が一致しない");
      return false;
    }
  }
  // ★2026-09-24 FW13: 削除→rename の間に電源断すると設定が消えるため、置換 rename にした。
  if (!InternalFS.rename(SETTINGS_TMP_FILE, SETTINGS_FILE)) { Serial.println("[SETTINGS] ✗ 差し替えに失敗"); return false; }
  Serial.println("[SETTINGS] flashへ保存しました（読み戻しで確認済み）");
  return true;
}
...
static int measureDS18B20(uint8_t ch) {
  ...
  if (t == DEVICE_DISCONNECTED_C) {
    ...
    s_errors |= ERR_DS18B20;  // ★2026-09-23 雛形: 欠測として判定できるようにする
    return 0;
  }
...
static float medianWithRange(float *a, int n, float *outRange) {
  // ★2026-09-23 雛形FW12: 作業配列を上限で確保する（Codexレビュー指摘#1）。
  //   メジアン回数をダウンリンクで変えられるようにしたのに、ここだけ初期値(5)の固定長のままで、
  //   6以上を指定すると配列の外へ書き込んでいた（v3.20 は MEASURE_COUNT_MAX で確保済み。移植漏れ）。
  if (n < 1) n = 1;
  if (n > MEASURE_COUNT_MAX) n = MEASURE_COUNT_MAX;
  float t[MEASURE_COUNT_MAX];
...
static bool hxReadAvg(float *outAvg) {
  float sum = 0;
  for (int i = 0; i < ACTIVE_SAMPLES_PER_AVG; i++) {  // ★2026-09-23 雛形: 実行時の平均回数
...
static bool hxRead(int *out, float *outRange = nullptr) {
  float avgs[MEASURE_COUNT_MAX];   // ★2026-09-23 雛形: 実行時の回数は可変なので上限で確保する
  const int n = ACTIVE_MEASURE_COUNT;
  for (int i = 0; i < n; i++) {
    if (!hxReadAvg(&avgs[i])) {
      *out = 0;
      if (outRange != nullptr) {
  ...
  float range = 0;
  *out = (int)medianWithRange(avgs, n, &range);
  if (outRange != nullptr) {
    *outRange = range;
  }
  return true;
...
// ============================================================
// MCP9600（熱電対、Kタイプ、I2C）— 温度[℃×10]を返す
//
// project06_yokogawa/ver1.1/src/main.cpp から移植。レジスタ定義・
// 初期化シーケンス（SENSORCONFIG=Kタイプ+フィルタ、DEVICECONFIG=0x80）は
// Yokogawa側と同一値。YokogawaはESP32-C3のI2Cクロックストレッチ制約
// （タイムアウトが5bitしかない既知の問題）を回避するため自作ビットバング
// I2C（soft_i2c.cpp）を使っていたが、XIAO nRF52840のTWIMには同様の制約が
// ないため、本ファイルでは他のI2Cデバイス（TCA9534/DS3231等）と同様に
// 標準の Wire（ハードウェアI2C）をそのまま使用する。
//
// I2Cアドレスは基板の実装により0x60〜0x67の範囲で変わるため、Yokogawa
// と同じくスキャンして検出する（固定アドレス決め打ちにしない）。
// TCA9546A 経由でチャンネル分離するため、他CHのI2Cデバイスとのアドレス
// 競合は起きない。3V3_SW サイクルごとに電源が切れるため毎サイクル初期化する。
// ============================================================

#define MCP9600_REG_HOTJUNCTION 0x00
#define MCP9600_REG_STATUS      0x04
#define MCP9600_REG_SENSORCFG   0x05
#define MCP9600_REG_DEVICECFG   0x06
#define MCP9600_REG_DEVICEID    0x20
#define MCP9600_STATUS_TH_UPDATE 0x40  // STATUSレジスタ bit6: 新しいHOTJUNCTION値が更新された
#define MCP9600_ADDR_MIN 0x60
#define MCP9600_ADDR_MAX 0x67
#define MCP9600_CONV_WAIT_TIMEOUT_MS 3000UL  // 変換完了待ちの上限（電源投入直後は長引くことがある）

// デバッグ用: MCP9600がI2Cバス上で見えているか確認するスキャンログ。
// 原因究明（I2Cクロック速度・書込間隔の問題）は解決済みのため通常は0のままでよい。
// 再度配線トラブル等が疑わしいときだけ1にすると、CH4のTCA9546Aチャンネル配下に
// 何が見えているかを確認できる。
#define MCP9600_DEBUG_SCAN 0

// 現在tcaSelect()で選択中のバス上でMCP9600をスキャンする。見つからなければ0を返す。
//
// 書き込みプローブ（beginTransmission+endTransmission、データ0バイト）だとNACKになる
// ことが実機で確認できたため、読み取りプローブ（requestFrom）を使う（個体によっては
// 空書き込みには応答せず読み取り要求には応答するため）。
static uint8_t mcp9600ScanOnce() {
  for (uint8_t addr = MCP9600_ADDR_MIN; addr <= MCP9600_ADDR_MAX; addr++) {
    uint8_t n = Wire.requestFrom(addr, (uint8_t)1);
    if (n >= 1) {
      while (Wire.available()) Wire.read();  // 読み捨て
      return addr;
    }
  }
  return 0;
...
// mcp9600ScanOnce() を最大3回リトライする（DS18B20のretryと同じ方式）。
// TCA9546A経由の配線・タイミングマージンによる一過性の揺らぎを想定してリトライする。
static uint8_t mcp9600Scan() {
  for (int retry = 0; retry < 3; retry++) {
    uint8_t addr = mcp9600ScanOnce();
...
// DEVICEID確認（0x40=MCP9600 / 0x41=MCP9601）→ Kタイプ設定 → DEVICECONFIG書込
static bool mcp9600Init(uint8_t addr) {
  Wire.beginTransmission(addr);
  Wire.write(MCP9600_REG_DEVICEID);
  if (Wire.endTransmission(false) != 0) return false;
  if (Wire.requestFrom(addr, (uint8_t)2) < 2) return false;
  uint8_t devId = Wire.read();
  Wire.read();  // Revision（未使用）
  if (devId != 0x40 && devId != 0x41) return false;  // 0x40=MCP9600 / 0x41=MCP9601

  // SENSORCONFIG: Kタイプ(bit6-4=000) + フィルタmid(bit2-0=011) = 0x03
  Wire.beginTransmission(addr);
  Wire.write(MCP9600_REG_SENSORCFG);
  Wire.write(0x03);
  if (Wire.endTransmission() != 0) return false;

  // SENSORCONFIG書込直後は内部処理でビジーになり、間髪入れずに次の通信をすると
  // NACKになる事象が実機で確認できたため、短い待ちを入れる。
  delay(20);

  // DEVICECONFIG: 0x80（Yokogawaと同一値。バーンアウト検出無効・通常動作モード）
  Wire.beginTransmission(addr);
  Wire.write(MCP9600_REG_DEVICECFG);
  Wire.write(0x80);
  return Wire.endTransmission() == 0;
}

// STATUSレジスタを読む（1バイト）。失敗時は0xFFを返す（bit6が立った状態＝誤検知の
// リスクがあるが、呼び出し側は endTransmission/requestFrom の成否を別途見ていないため、
// このヘルパー内で完結させるための便宜上の値）。
static uint8_t mcp9600ReadStatus(uint8_t addr) {
  Wire.beginTransmission(addr);
  Wire.write(MCP9600_REG_STATUS);
  if (Wire.endTransmission(false) != 0) return 0xFF;
  if (Wire.requestFrom(addr, (uint8_t)1) < 1) return 0xFF;
  return Wire.read();
}

static void mcp9600ClearStatus(uint8_t addr, uint8_t status) {
  Wire.beginTransmission(addr);
  Wire.write(MCP9600_REG_STATUS);
  Wire.write((uint8_t)(status & ~MCP9600_STATUS_TH_UPDATE));
  Wire.endTransmission();
}

// 新しい変換完了（Temperature Updateフラグ）を待つ。
// 電源投入直後はフィルタが収束するまで通常より時間がかかることがあるため、
// 固定delay()ではなくフラグをポーリングして「準備できた分だけ待つ」方式にしている。
// 手順: まず既存のフラグを一度クリアし、その後に新しく立つのを待つ
// （クリア前から立っていたフラグ＝電源投入前の古い値を拾わないため）。
static bool mcp9600WaitForConversion(uint8_t addr, uint32_t timeoutMs) {
  uint8_t status = mcp9600ReadStatus(addr);
  if (status != 0xFF) mcp9600ClearStatus(addr, status);

  uint32_t t0 = millis();
  while (millis() - t0 < timeoutMs) {
    status = mcp9600ReadStatus(addr);
    if (status != 0xFF && (status & MCP9600_STATUS_TH_UPDATE)) {
      mcp9600ClearStatus(addr, status);
      return true;
    }
    delay(50);
  }
...
static float mcp9600ReadTemp(uint8_t addr) {
  Wire.beginTransmission(addr);
  Wire.write(MCP9600_REG_HOTJUNCTION);
  if (Wire.endTransmission(false) != 0) return NAN;
  if (Wire.requestFrom(addr, (uint8_t)2) < 2) return NAN;
  uint8_t hi = Wire.read();
  uint8_t lo = Wire.read();
  return (int16_t)((hi << 8) | lo) * 0.0625f;  // MCP9600分解能: 0.0625℃/LSB
}

// CH_ASSIGN=5 用。戻り値: 温度×10（例: 253 = 25.3℃）、失敗時 0
// project06_yokogawa/ver1.1（本番稼働実績あり）はMCP9600/9601アクセスに
// 自作ビットバングI2C（約10kHz相当、標準の100kHzより大幅に遅い）を使っている。
// 実機でハードウェアWire(nRF52 TWIM、通常100kHz)からのアクセスがNACK(err=2)で
// 一貫して失敗する事象が確認できたため、Yokogawaの実績値に合わせてMCP9600アクセス
// 中だけI2Cクロックを10kHzへ落とす（TCA9546A経由の配線・容量負荷でACKタイミングに
// 間に合っていない可能性が高い）。他デバイスへの影響を避けるため、アクセス後は
// 通常速度(100kHz)へ戻す。
#define MCP9600_I2C_CLOCK_HZ   10000UL
#define I2C_DEFAULT_CLOCK_HZ   100000UL

static int measureThermocouple() {
  Wire.setClock(MCP9600_I2C_CLOCK_HZ);

  uint8_t addr = mcp9600Scan();
  if (addr == 0) {
    s_errors |= ERR_MCP9600_I2C;
    statusErrorRed();
    Serial.println("[MCP9600] not found (0x60-0x67)");
    Wire.setClock(I2C_DEFAULT_CLOCK_HZ);
    return 0;
  }
  if (!mcp9600Init(addr)) {
    s_errors |= ERR_MCP9600_I2C;
    statusErrorRed();
    Serial.print("[MCP9600] init failed (addr=0x");
    Serial.print(addr, HEX);
    Serial.println(")");
    Wire.setClock(I2C_DEFAULT_CLOCK_HZ);
    return 0;
  }
  // 固定delay()ではなく、変換完了フラグが立つまで待つ（電源投入直後は
  // フィルタ収束に時間がかかることがあり、認識に「少し時間がかかる」印象の原因）。
  if (!mcp9600WaitForConversion(addr, MCP9600_CONV_WAIT_TIMEOUT_MS)) {
    Serial.println("[MCP9600] 変換完了待ちタイムアウト（それでも読み取りを試みます）");
  }

  float t = mcp9600ReadTemp(addr);
  Wire.setClock(I2C_DEFAULT_CLOCK_HZ);
  if (isnan(t)) {
    s_errors |= ERR_MCP9600_I2C;
    statusErrorRed();
    Serial.println("[MCP9600] read failed");
    return 0;
  }

  Serial.print("[MCP9600] "); Serial.print(t, 2); Serial.println(" degC");
  return (int)(t * 10.0f);
}

// ============================================================
// ADS1115（電圧入力、±10V相当）— 電圧[mV]を返す
//
// project06_yokogawa/ver1.1/src/main.cpp から移植。外部±10V入力を分圧回路で
// ADS1115のPGA±2.048Vレンジに収め、実測較正した GAIN/OFFSET で元の電圧に
// 逆変換する方式もYokogawa側と同一。差動入力A0-A1（チャンネルA）と
// A2-A3（チャンネルB）をconfigレジスタの切替で使い分け、1個のADS1115で
// 電圧入力2ch分をまかなう（CH_ASSIGN=6/7の2スロットに対応）。
//
// ★GAIN/OFFSETはYokogawa基板での実測較正値をそのまま転記した暫定値。
//   分圧抵抗の実装値・個体差がFlex基板側と異なる可能性が高いため、
//   既知の基準電圧源で実測し、Flex基板専用の値に再校正すること。
// ============================================================

#define ADS1115_ADDR 0x48
#define ADS_REG_CONV 0x00
#define ADS_REG_CFG  0x01
static const uint16_t ADS_CFG_CHA = 0x8583;  // A0-A1差動 / PGA±2.048V / single-shot / 128SPS
static const uint16_t ADS_CFG_CHB = 0xB583;  // A2-A3差動 / 他は同上
static const float ADS_GAIN_CHA   = 7.5407f;  // ★Yokogawa実測値。Flex基板で要再校正
static const float ADS_OFFSET_CHA = 0.086f;   // ★同上
static const float ADS_GAIN_CHB   = 7.5658f;  // ★同上
static const float ADS_OFFSET_CHB = 0.067f;   // ★同上

static int16_t adsReadRaw(uint16_t config) {
  ...
  delay(15);  // 128SPS換算の変換時間+余裕（Yokogawaと同一値）
  uint32_t deadline = millis() + 50;
  while (millis() < deadline) {
    Wire.beginTransmission(ADS1115_ADDR);
    Wire.write(ADS_REG_CFG);
    if (Wire.endTransmission(false) != 0) return INT16_MIN;
    if (Wire.requestFrom(ADS1115_ADDR, (uint8_t)2) < 2) return INT16_MIN;
    uint8_t hi = Wire.read();
    Wire.read();  // lo（OS bit判定には不要）
    if (hi & 0x80) break;  // OS bit=1: 変換完了
    delay(2);
  }
...
static float adsReadVoltage(uint16_t config, float gain, float offset) {
  int16_t raw = adsReadRaw(config);
  if (raw == INT16_MIN) return NAN;
  float v_ads = raw * (2.048f / 32768.0f);  // PGA±2.048VレンジのLSB換算
  return v_ads * gain + offset;             // 分圧回路を元の電圧へ逆変換
}

// CH_ASSIGN=6(channelB=false)/7(channelB=true) 用。戻り値: 電圧[mV]、失敗時 0
static int measureVoltage(bool channelB) {
  uint16_t cfg    = channelB ? ADS_CFG_CHB    : ADS_CFG_CHA;
  float    gain   = channelB ? ADS_GAIN_CHB   : ADS_GAIN_CHA;
  float    offset = channelB ? ADS_OFFSET_CHB : ADS_OFFSET_CHA;

  float v = adsReadVoltage(cfg, gain, offset);
  if (isnan(v)) {
    s_errors |= ERR_ADS1115_I2C;
    statusErrorRed();
    Serial.print("[ADS1115] ch"); Serial.print(channelB ? "B" : "A");
    Serial.println(" read failed");
    return 0;
  }

  Serial.print("[ADS1115] ch"); Serial.print(channelB ? "B" : "A");
  Serial.print(" "); Serial.print(v, 3); Serial.println(" V");
  return (int)(v * 1000.0f);  // mV単位の整数値として返す
}
...
// ★2026-09-23 雛形: チャネルごとの有効/欠測。false のチャネルは LoRa で欠測値(0x8000)を送る。
//   従来は読み取り失敗でも 0 を送っており、本当に 0 だったのか区別できなかった。
static bool s_chValid[4];

// 1チャネル分の計測を行い、計測中にチャネル単位のエラーが立ったら欠測にする。
//   s_errors は1サイクル中は立ったまま（sticky）なので、計測の前に一旦外して判定し、後で戻す。
#define MEASURE_CH(idx, expr) do { \
    uint32_t _saved = s_errors & CH_ERR_MASK; s_errors &= ~CH_ERR_MASK; \
    ch[idx] = (expr); \
    if ((s_errors & CH_ERR_MASK) != 0U) s_chValid[idx] = false; \
    s_errors |= _saved; } while (0)
...
static void measureAll() {
  ...
  for (int i = 0; i < 4; i++) s_chValid[i] = (CH_ASSIGN[i] != 0);  // ★2026-09-23 雛形: 未使用は欠測
  ...
  for (int i = 0; i < 4; i++) {
    if (CH_ASSIGN[i] == 3) {
      MEASURE_CH(i, measureDS18B20((uint8_t)(i + 1)));
      Serial.print("[CH");
      Serial.print(i + 1);
      Serial.print("] ");
      Serial.println(ch[i]);
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (CH_ASSIGN[i] == 1) {
      if ((s_errors & ERR_TCA9534_I2C) != 0U) {
        ch[i] = 0;
        s_chValid[i] = false;  // ★2026-09-23 雛形: 
#if defined(COMM_MODE_BLE) || defined(COMM_MODE_LORA)
        chRange[i] = 0;
#endif
      } else {
        ...
        {
          float range = 0;
          if (!hxRead(&ch[i], &range)) s_chValid[i] = false;  // ★2026-09-23 雛形: 
          // レンジも生値→ひずみ値（με）変換で単位を揃える
          chRange[i] = (int)(range / STRAIN_SCALE);
        }
#else
        if (!hxRead(&ch[i])) s_chValid[i] = false;  // ★2026-09-23 雛形: 
#endif
        // 生値 → ひずみ値（με）変換
      ...
      if (tcaSelect((uint8_t)i) != 0) {
        ...
        MEASURE_CH(i, measureVL53L4CD());
      }
      tcaDisable();
    } else if (CH_ASSIGN[i] == 5) {
      // TCA9546A のチャネル i（0 origin）を選択して MCP9600（熱電対）を読む
      if (tcaSelect((uint8_t)i) != 0) {
        s_errors |= ERR_TCA_I2C;
        statusErrorRed();
        ch[i] = 0;
        ...
        // ★検証用に大幅に伸ばした待ち時間（50ms→500ms）。
        // scanI2CBus()（アドレス1〜126を総当たり、ソフトウェア的な所要時間が
        // 数百msオーダーになりうる）の直後だとMCP9600が検出できるが、
        // 切替直後すぐだと検出できない実機事例があるため、単純な時間経過が
        // 効いているのかを大きめの値で切り分ける。
        delay(500);
#if MCP9600_DEBUG_SCAN
        // ★一時デバッグ: このCHのTCA9546Aチャネル配下に何が見えているか確認する
        {
          char label[16];
          snprintf(label, sizeof(label), "CH%d (TCA ch%d)", i + 1, i);
          scanI2CBus(label);
        }
        // ★重要: scanI2CBus()はアドレス1〜126を総当たりする過程でTCA9546A自身の
        //   アドレス(0x70)にも「データなしの書き込み」を送ってしまい、これが
        //   チャンネル選択を意図せず解除している可能性がある。前回この再選択を
        //   入れた状態と、その後delay()を試すために外してしまった状態を比較すると
        //   再選択ありの方でのみ検出できていた可能性があるため、確実に戻す。
        tcaSelect((uint8_t)i);
#endif
        MEASURE_CH(i, measureThermocouple());
      }
      tcaDisable();
    } else if (CH_ASSIGN[i] == 6 || CH_ASSIGN[i] == 7) {
      // TCA9546A のチャネル i（0 origin）を選択して ADS1115（電圧）を読む
      // CH_ASSIGN=6→チャンネルA(A0-A1) / 7→チャンネルB(A2-A3)
      if (tcaSelect((uint8_t)i) != 0) {
        s_errors |= ERR_TCA_I2C;
        statusErrorRed();
        ch[i] = 0;
        ...
        MEASURE_CH(i, measureVoltage(CH_ASSIGN[i] == 7));
      }
      tcaDisable();
    } else {
...
// ★2026-09-23 雛形: ここから firmware_child（v3.20系）のダウンリンク受信を移植（Class A 方式）。
//   Gateway はアップリンク検知から400ms待ってから 0x81 を返すので、送信直後に受信窓を開く。
...
#define DL_STATUS_SAVE_ERROR  3   // ★2026-09-23 雛形FW12: flash への保存に失敗（設定は変更していない）。Gateway・GAS と一致させること
...
static bool applyDownlinkPayload(const uint8_t *payload, uint8_t len) {
  ...
  // ★2026-09-23 雛形FW12: 全項目の値域を先に検証し、すべて正しいときだけまとめて適用する（Codexレビュー指摘#7）。
  //   旧実装は送信間隔を先に書き換えてからメジアン回数を検証していたため、
  //   「間隔3分・メジアン21」を送ると間隔だけ変わって保存・再起動するのに、GAS には「拒否」と出ていた。
  ChildSettings next = s_settings;
  bool rangeOk = true;
  ...
  if (flags & DL_FLAG_AVG_MEDIAN) {
    ...
    if (newAvg == 0 || newMedian == 0) {
      ...
      next.samplesPerAvg = newAvg;
      next.measureCount  = newMedian;
    }
  }

  if (!rangeOk) {
    status = DL_STATUS_RANGE_ERROR;
    Serial.println("[DOWNLINK] 値域エラーがあるため、設定は1つも変更しません");
  } else if (memcmp(&next, &s_settings, sizeof(next)) != 0) {
    const ChildSettings prev = s_settings;
    sleepChanged = (next.sleepMinutes != prev.sleepMinutes);
    s_settings = next;
    if (saveSettings()) {
      changed = true;
      Serial.print("[DOWNLINK] 設定を変更: 間隔="); Serial.print(s_settings.sleepMinutes);
      Serial.print("分 平均="); Serial.print(s_settings.samplesPerAvg);
      Serial.print(" メジアン="); Serial.println(s_settings.measureCount);
    } else {
      // 保存できなかったので元に戻す。再起動もしない（再起動すると保存前の値に戻り、GASと食い違う）
      s_settings = prev;
      sleepChanged = false;
      status = DL_STATUS_SAVE_ERROR;
      Serial.println("[DOWNLINK] ✗ flashへの保存に失敗したため、設定は変更しません");
    }
  }
  (void)changed;
...
// ============================================================
// LoRa ペイロード 0x06（可変長）  ★2026-09-23 雛形で新設
// ============================================================
//   [0]            PktType = 0x06
//   [1]            DeviceID
//   [2]            FW Version
//   [3]            n = チャネル数（1〜12。Flex は最大4）
//   [4 .. 3+2n]    CH1〜CHn（int16 LE）。0x8000 = 欠測（未使用・読み取り失敗）
//   [4+2n .. 5+2n] BATT（uint16 LE, mV）
//   [6+2n]         Hour（0〜23。0xFF = 時刻無効）
//   [7+2n]         Min （0〜59。0xFF = 時刻無効）
//   長さ = 8 + 2n バイト
//
// 【0x04（19バイト固定）から変えた理由】
//   ・チャネル数を子機が決められる（Gateway は中身を見ずにそのまま運べる）
//   ・欠測（0x8000）と時刻無効（0xFF）を、本物の 0 や 0:0 と区別できる
//     （0x04 ではセンサー失敗が CH=0、RTC 失敗が TIME=0:0 になり、もっともらしい値に化けていた）
//   ・CH Range（4バイト）は Gateway・GAS のどこでも使っていなかったので外した
#define LORA_PKT_TYPE_VAR   0x06
#define LORA_CH_MISSING     ((int16_t)0x8000)
#define LORA_TIME_INVALID   0xFF
static_assert(CFG_SEND_CH >= 1 && CFG_SEND_CH <= 4, "CFG_SEND_CH must be 1..4 (Flex has 4 slots)");

static void sendLoRa() {
  // ★一時的な緩和（iPEC疎通テスト用）: センサーエラーがあっても送信は続ける。
  //   0x06 では失敗したチャネルは欠測(0x8000)として送るので、受信側で区別できる。
  if (s_errors != 0U) {
    Serial.print("[LORA] errors present (0x");
    Serial.print(s_errors, HEX);
    Serial.println(") だが送信を続行（失敗したチャネルは欠測として送る）");
  }
  ...
  // 年が明らかにおかしい（電池切れで 2000 年に戻った等）場合も時刻無効として送る
  if (rtcOk && (rtcT.year < 2026 || rtcT.hour > 23 || rtcT.min > 59)) rtcOk = false;

  const uint8_t n = CFG_SEND_CH;
  uint8_t msd[8 + 2 * 4];
  msd[0] = LORA_PKT_TYPE_VAR;
  msd[1] = DEVICE_ID;
  msd[2] = FW_VERSION;
  msd[3] = n;
  for (uint8_t i = 0; i < n; i++) {
    // ★2026-09-23 雛形FW12: int16 に収まらない値は、縮めると符号が反転した別の値になる（例: 40000 → -25536）。
    //   範囲外は欠測として送る（Codexレビュー指摘#9）。
    bool valid = s_chValid[i];
    if (valid && (ch[i] > 32767 || ch[i] < -32767)) {
      valid = false;
      Serial.print("[LORA] CH"); Serial.print(i + 1); Serial.print(" の値 "); Serial.print(ch[i]);
      Serial.println(" は int16 の範囲外のため欠測として送る");
    }
    int16_t v = valid ? (int16_t)ch[i] : LORA_CH_MISSING;
    if (valid && v == LORA_CH_MISSING) v = (int16_t)(LORA_CH_MISSING + 1);  // 本物の値が欠測値と衝突しないように
    msd[4 + i * 2]     = (uint8_t)(v & 0xFF);
    msd[4 + i * 2 + 1] = (uint8_t)((v >> 8) & 0xFF);
  }
  uint16_t batt = (uint16_t)battV;
  msd[4 + 2 * n] = (uint8_t)(batt & 0xFF);
  msd[5 + 2 * n] = (uint8_t)((batt >> 8) & 0xFF);
  msd[6 + 2 * n] = rtcOk ? rtcT.hour : LORA_TIME_INVALID;
  msd[7 + 2 * n] = rtcOk ? rtcT.min  : LORA_TIME_INVALID;
  const uint8_t msdLen = (uint8_t)(8 + 2 * n);

  statusSigfoxBlinkReset();
  for (uint8_t rep = 0; rep < LORA_TX_REPEAT; rep++) {
    loraSendFrame(msd, msdLen);
    delay(LORA_TX_COMPLETE_DELAY_MS);  // 送信完了待ち（電源断・次送信前に確保する）
    if (rep + 1 < LORA_TX_REPEAT) delay(LORA_TX_REPEAT_GAP_MS);
  }

  Serial.print("[LORA] TX 0x06 FW="); Serial.print(FW_VERSION);
  Serial.print(" n="); Serial.print(n);
  Serial.print(" CH=");
  for (uint8_t i = 0; i < n; i++) {
    if (s_chValid[i]) Serial.print(ch[i]); else Serial.print("欠測");
    Serial.print(" ");
  }
  Serial.print("BATT="); Serial.print(battV);
  Serial.print(" TIME=");
  if (rtcOk) { Serial.print(rtcT.hour); Serial.print(":"); Serial.println(rtcT.min); }
  else Serial.println("無効");

  // 送信直後にダウンリンクの受信窓を開く（Class A）。送信で E220 の RF 受信が止まるので先に復活させる。
  loraReviveRx();
...
#ifdef COMM_MODE_LORA
  loadSettings();  // ★2026-09-23 雛形: ダウンリンクで変えた設定を復元。必ず wdtInit より前に呼ぶ
#endif
  wdtInit(computeWdtTimeoutMs());  // 無人運用の安全網。送信間隔＋WDT_MARGIN_MINUTES でリセット
  Serial.print("[WDT] タイムアウト="); Serial.print(computeWdtTimeoutMs() / 60000UL); Serial.println("分");
...
#ifdef COMM_MODE_LORA
  sendLoRa();
  deepSleep(ACTIVE_SLEEP_MINUTES);  // ★2026-09-23 雛形: 実行時の送信間隔
#endif
