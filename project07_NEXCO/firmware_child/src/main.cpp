/**
 * project07_NEXCO — 剥落検知センサ ファームウェア（PlatformIO / XIAO nRF52840）
 * ベース: Monita Flex v3.03（COMM_MODE_BLE で運用）
 *
 * 【案件】中国自動車道有野川付近RC床版 コンクリート剥落モニタリング
 * 【センサ】HX711×4ch（ひずみゲージ、片持ち梁・2ゲージ法）
 *   ch1: 剥落エリア左 / ch2: 中央 / ch3: 右 / ch4: 参照点（温度補償）
 *
...
// HX711 計測パラメータ
// SAMPLES_PER_AVG: 1回の平均を求めるためのサンプル数
// MEASURE_COUNT  : 平均値を何回取得してメジアン/最大/最小を求めるか
// 所要時間 ≈ SAMPLES_PER_AVG × MEASURE_COUNT × (1000ms / HX711_RATE_HZ) × 4ch
//           = 5 × 10 × 12.5ms × 4ch ≈ 2.5秒（MUX切替オーバーヘッド除く）
#define SAMPLES_PER_AVG  5   // 1回の平均サンプル数（HX711 80Hz時: 5個 = 62.5ms）
#define MEASURE_COUNT   10   // 平均値の取得回数（メジアン/最大/最小の母数）
...
// float 配列をバブルソートしてソート済み配列を返す（インプレース）
static void sortFloats(float *a, int n) {
  for (int i = 0; i < n - 1; i++)
    for (int j = 0; j < n - i - 1; j++)
      if (a[j] > a[j + 1]) { float x = a[j]; a[j] = a[j + 1]; a[j + 1] = x; }
}

// n 個の float 配列からメジアンを返す（配列はソートされる）
...
static bool hxReadAvg(float *outAvg) {
  ...
  for (int i = 0; i < SAMPLES_PER_AVG; i++) {
    sum += hx.get_value();  // get_value() = read() - tare_offset（タレ補正済み）
  }
  *outAvg = sum / SAMPLES_PER_AVG;
  return true;
}

// MEASURE_COUNT 回の平均値を取得し、メジアン・最大・最小を返す。
// outMax / outMin が非 NULL の場合のみ書き込む（BLE モード時に使用）。
// 戻り値: true=正常、false=いずれかの計測でタイムアウト
static bool hxRead(int *out, int *outMax = nullptr, int *outMin = nullptr) {
  ...
  // ソート済み配列からメジアン・最大・最小を取得
  float sorted[MEASURE_COUNT];
  memcpy(sorted, avgs, sizeof(avgs));
  sortFloats(sorted, MEASURE_COUNT);
  *out    = (int)sorted[MEASURE_COUNT / 2];        // メジアン（生値）
  if (outMax) *outMax = (int)sorted[MEASURE_COUNT - 1];  // 最大
  if (outMin) *outMin = (int)sorted[0];                  // 最小
  return true;
}
...
// ch[0..3]: 各スロットの計測値（ひずみ[με]。HX711メジアン値）
int ch[4], tempV, battV;

#ifdef COMM_MODE_BLE
// chMax/chMin: MEASURE_COUNT回の平均値群からの最大・最小 [με]
int chMax[4];
int chMin[4];
#endif
...
#ifdef COMM_MODE_BLE
        {
          int rawMax = 0, rawMin = 0;
          hxRead(&ch[i], &rawMax, &rawMin);
          chMax[i] = (int)((float)rawMax / STRAIN_SCALE);
          chMin[i] = (int)((float)rawMin / STRAIN_SCALE);
        }
#else
        hxRead(&ch[i]);
#endif
...
//
// MSD フォーマット（29 バイト・BLE上限ぴったり）:
//   [0-1]   Company ID  : 0xFF 0xFF
//   [2]     Pkt type    : 0x03（Monita Flex v3.03）
//   [3]     Device ID   : DEVICE_ID
//   [4]     温度        : int8_t（DS3231・整数℃。精度±3℃のため1℃分解能で十分）
//   [5-6]   CH1 メジアン: int16_t LE [με]（MEASURE_COUNT回の平均値群のメジアン）
//   [7-8]   CH2 メジアン: int16_t LE [με]
//   [9-10]  CH3 メジアン: int16_t LE [με]
//   [11-12] CH4 メジアン: int16_t LE [με]
//   [13-14] CH1 Max     : int16_t LE [με]
//   [15-16] CH1 Min     : int16_t LE [με]
//   [17-18] CH2 Max     : int16_t LE [με]
//   [19-20] CH2 Min     : int16_t LE [με]
//   [21-22] CH3 Max     : int16_t LE [με]
//   [23-24] CH3 Min     : int16_t LE [με]
//   [25-26] CH4 Max     : int16_t LE [με]
//   [27-28] CH4 Min     : int16_t LE [με]  ← buf[27-28]、29バイト配列の末尾 ✅
//
// 計測方式: SAMPLES_PER_AVG サンプル平均 × MEASURE_COUNT 回 → メジアン/最大/最小
// ============================================================
#ifdef COMM_MODE_BLE
static void bleAdvertise() {
  uint8_t buf[29];  // BLE MSD 上限: CompanyID(2) + data(27) = 29B

  auto writeI16 = [&](int idx, int val) {
  ...
  buf[2] = 0x03;                          // Pkt type
  buf[3] = DEVICE_ID;
  buf[4] = (uint8_t)(int8_t)(tempV / 10); // 温度 整数℃（int8_t → uint8_t）
  for (int i = 0; i < 4; i++) writeI16(5  + i * 2, ch[i]);     // メジアン [5-12]
  for (int i = 0; i < 4; i++) writeI16(13 + i * 4, chMax[i]);  // Max [13,17,21,25]
  for (int i = 0; i < 4; i++) writeI16(15 + i * 4, chMin[i]);  // Min [15,19,23,27]

  Bluefruit.Advertising.stop();
  Bluefruit.Advertising.clearData();
  Bluefruit.ScanResponse.clearData();
  // ★Flags(3B)を足すとMSD(31B)と合わせて34B超過し、Legacy ADVの31B上限をオーバーする
  // （start()の戻り値未チェックのため無音で失敗していた）。Gateway側はFlagsを見ないため削除。
  ...
  Serial.print(" TEMP="); Serial.print((int8_t)(tempV / 10)); Serial.print("℃");
  Serial.print(" CH(med)=");
  for (int i = 0; i < 4; i++) { Serial.print(ch[i]); Serial.print(" "); }
  Serial.print("MAX=");
  for (int i = 0; i < 4; i++) { Serial.print(chMax[i]); Serial.print(" "); }
  Serial.print("MIN=");
  for (int i = 0; i < 4; i++) { Serial.print(chMin[i]); Serial.print(" "); }
  Serial.println();
#endif
}
#endif  // COMM_MODE_BLE
