/**
 * project07_NEXCO Gateway — BLE 受信 + LTE-M → GAS 送信（AC電源版）
 * ベース: Monita Gateway v1.1
 * 案件: 中国自動車道有野川付近RC床版 コンクリート剥落モニタリング
 *
 * 【ペイロード変更点（子機 firmware との対応）】
 *   子機フォーマット v3.03 NEXCO版（29B）:
 *   [0-1]=CompanyID [2]=PktType [3]=DeviceID
 *   [4-5]=温度×10  [6-13]=CH1-4メジアン  [14-21]=CH1-4 Max  [22-29]=CH1-4 Min
 *
...
const char* GAS_SCRIPT_ID = "AKfycbwG3YGZoqm1vCwKYkLRnV13oxhthx0ZXbLyi0zVV6kkowIv72A_wkCYh2IU6GsiDFTL/exec";
...
// 0 = 無効（通常運用時は必ず 0 に戻すこと）。
#define TEST_INJECT_FAKE_DEVICE_COUNT 0  // ★テスト用に10に設定中。検証後は必ず0に戻すこと
...
// project07_NEXCO 子機ペイロード: Company ID(2B) を除くと 27 バイト
// [0]=PktType [1]=DeviceID [2-3]=温度 [4-11]=CH1-4メジアン [12-19]=CH1-4 Max [20-27]=CH1-4 Min
// 拡張余地を含め 32 バイトを確保する。
...
int buildBatchQuery(const FlexRecord* merged, int start, int n,
  ...
  // project07_NEXCO ペイロード構造（子機 MSD から CompanyID を除いた後）:
  //   payload[0]=PktType  [1]=DeviceID  [2]=温度(int8_t,整数℃)
  //   [3-10]=CH1-4メジアン(int16_t LE×4)
  //   [11-12]=CH1Max [13-14]=CH1Min [15-16]=CH2Max [17-18]=CH2Min
  ...
  for (int i = start; i < n; i++) {
    ...
    snprintf(chunk, sizeof(chunk),
             ...
             "%02X%02X%02X%02X"                               // CH1 Max(2B) + CH1 Min(2B)
             ...
             rec.payload[5],  rec.payload[6],                  // CH2 med
             rec.payload[7],  rec.payload[8],                  // CH3 med
             rec.payload[9],  rec.payload[10],                 // CH4 med
             rec.payload[11], rec.payload[12],                 // CH1 Max
             rec.payload[13], rec.payload[14],                 // CH1 Min
             rec.payload[15], rec.payload[16],                 // CH2 Max
             rec.payload[17], rec.payload[18],                 // CH2 Min
             rec.payload[19], rec.payload[20],                 // CH3 Max
             rec.payload[21], rec.payload[22],                 // CH3 Min
             rec.payload[23], rec.payload[24],                 // CH4 Max
