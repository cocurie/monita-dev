...
public:
  ...
  bool resident(uint32_t idx) const {
  ...
  bool get(uint32_t idx, int32_t out[NUM_CH]) const {
    ...
    for (uint8_t c = 0; c < NUM_CH; ++c) {
      ...
      out[c] = (raw & 0x800000u) ? (int32_t)(raw | 0xFF000000u) : (int32_t)raw;
      p += 3;
    }
    return true;
  }
...
// ─────────────────────────────────────────────────────────────
struct EventWindow {
  uint32_t triggerIdx = 0;   // 発火した絶対サンプル番号
  uint32_t startIdx   = 0;   // 切出し開始（= triggerIdx - preSamples）
  uint32_t endIdx     = 0;   // 切出し終了（含まない）
...
public:
   ...
   */
  bool update(uint32_t idx, const int32_t ch[NUM_CH], EventWindow& win);
...
private:
  ...
  uint32_t overSince_ = 0;     // 条件成立が始まった絶対番号
  bool     over_ = false;
  uint32_t deadUntil_ = 0;     // この番号までは再発火しない
