...
int16_t packPirScanAndCharge(uint32_t scanCount, uint8_t chargeState) {
  // スキャン回数は1報告あたり最大1440回（報告間隔1440分×毎分1回）なので13bitで足りる。
  // 上限3<<13|8191 = 32767 でint16の正の範囲に収まり、欠測値-1とも衝突しない。
  const uint32_t scans = scanCount > PIR_SCAN_COUNT_MAX ? PIR_SCAN_COUNT_MAX : scanCount;
  return static_cast<int16_t>(scans |
      (static_cast<uint32_t>(chargeState & 0x03U) << CHARGE_STATE_SHIFT));
}
...
void buildSensorLoRaPayload(const SensorPayloadInput &input,
  ...
  // One標準版は1CH直結。未使用CHは欠測として明示し、CH4に充電状態を載せる。
  putInt16Le(out + 5, MISSING_VALUE);
  putInt16Le(out + 7, MISSING_VALUE);
  putInt16Le(out + 9, static_cast<int16_t>(input.chargeState & 0x03U));
...
void buildPirLoRaPayload(const PirPayloadInput &input,
  ...
  putInt16Le(out + 9, packPirScanAndCharge(input.scanCount, input.chargeState));
  putUint16Le(out + 11, input.batteryMv);
...
void buildSensorSigfoxPayload(const SensorPayloadInput &input,
  ...
  putInt16Le(out + 6, static_cast<int16_t>(input.chargeState & 0x03U));
...
void buildPirSigfoxPayload(const PirPayloadInput &input,
  ...
  putInt16Le(out + 6, packPirScanAndCharge(input.scanCount, input.chargeState));
