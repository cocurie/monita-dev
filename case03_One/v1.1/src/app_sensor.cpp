/*
 * Monita One v1.1 標準センサ版
 * Flex v3.20の起床→1CH計測→送信→Tickless Idleを、One直結配線へ移植したもの。
 * v1.1: 充電モジュールのCHRG/DONEをログへ出す（ペイロードは v1.00 と同一）。
 */
#include <Arduino.h>
...
bool tareHx711() {
  s_hx.begin(ONE_DOUT_PIN, ONE_PD_SCK_PIN);
  // 直前の計測でpower_down()済み（PD_SCK=HIGH）。measureHx711()と同じく明示的に復帰させる。
  s_hx.power_up();
  delay(1);
  int64_t sum = 0;
...
void setup() {
  // ★halBegin()を最初に呼ぶ。コアのinitVariant()がVBAT_ENABLEをHIGHにしており、
  // LOWへ戻すまでの時間を最短にするため（完全な対策はvariant側の修正が必要・未決）。
  one::halBegin();
  Serial.begin(115200);
  loadSettings();
...
void loop() {
  ...
  const one::ChargeState charge = one::readChargeState();
  ...
  Serial.print(" charge=");     Serial.print(one::chargeStateName(charge));
  ...
  (void)measured;
  sendReport(channel, range, battery, temperature, charge);
